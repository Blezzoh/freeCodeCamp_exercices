...
argument is { last: "Capulet" }, then you must return the third object from 
the array (the first argument), because it contains the property and its value,
that was passed on as the second argument.
*/


function hasProperties(obj, source){
  ...
  for(var i = 0; i< keysArr.length; i++){
    if( !(obj.hasOwnProperty(keysArr[i])))
      ...
      if(obj[keysArr[i]] != source[keysArr[i]]){
        return false;
      }
    }
  }
...
function whatIsInAName(collection, source) {
  ...
  for(var i =0; i<collection.length; i++){
    if(hasProperties(collection[i], source)){
